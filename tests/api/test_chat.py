...
class TestChatConsumer:
    ...
    async def test_authentication(self, db):
        ...
        user = await sync_to_async(User.objects.create_user)(
            email='test@gmail.com', username='testuser', password='testpassword'
        )
        ...
        communicator = WebsocketCommunicator(application, f"/ws/chat/blah?token={token}")
    ...
    async def test_invalid_token(self, db):
        ...
        communicator = WebsocketCommunicator(application, "/ws/chat/?token=invalidtoken")
    ...
    async def test_message_broadcast(self, db):
        ...
        user1 = await sync_to_async(User.objects.create_user)(
            email='test1@gmail.com', username='user1', password='password123'
        )
        user2 = await sync_to_async(User.objects.create_user)(
            email='test2@gmail.com', username='user2', password='password123'
        )
        token1 = await self.generate_jwt_token(user1)
        token2 = await self.generate_jwt_token(user2)

        # Connect both users to the same room
        communicator1 = WebsocketCommunicator(application, f"/ws/chat/room1/?token={token1}")
        communicator2 = WebsocketCommunicator(application, f"/ws/chat/room1/?token={token2}")
